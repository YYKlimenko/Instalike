...
def write_to_database(instance: SQLModel, return_instance=True):
...
def retrieve_list_from_database(model: object):
    with Session(engine) as session:
        instances = session.query(model)
    return instances


def retrieve_instance_from_database(model: object, instance_id: int):
    with Session(engine) as session:
        instance = session.query(model).get(instance_id)
    return instance


def retrieve_instance_by_username(model: object, username: str):
    with Session(engine) as session:
        instances = retrieve_list_from_database(model)
...
def retrieve_from_database_by_user(model, user_id: int):
    with Session(engine) as session:
        instances = retrieve_list_from_database(model)
        instance = instances.filter(model.user_id == user_id)
    return instance.all()
