...
def post_image(file: UploadFile, text: str = Form(), token: str = Depends(handle_auth),
               session: Session = Depends(get_session)):
    return services.create_image(file, text, session, user_id=token['sub'])
...
def delete_image(image_id: int, token: str = Depends(handle_auth), session: Session = Depends(get_session)):
    ...
    services.delete_image(image_id, session)


@microblog.put('/api/v1/images/{image_id}/',
               status_code=201,
